...
|--------|-------------|:--------:|
| [CVE-2026-XXXX](https://github.com/seriousm4x/UpSnap/security/advisories/GHSA-w4jr-728f-5jhq) | UpSnap (>= 4.4.1, <= 5.3.5). The `POST /api/upsnap/init-superuser` endpoint is exposed without authentication, setup token validation, IP restriction, or rate limiting, allowing an unauthenticated attacker to register the initial superuser account on a fresh installation and obtain a privileged JWT. The stolen superuser token can then be abused to create a malicious device entry with an attacker-controlled `wake_cmd` value, which is later executed through `/bin/sh -c` by the server process, resulting in unauthenticated remote code execution as root in the default Docker deployment | Critical |
